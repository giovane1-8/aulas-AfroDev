# estudar duas bibliotecas: 
#unittest
#pytest

"""
    babystaps 
São os passos escritos
...
while True:
    try: #inicio de verificação por erro
        ...
        while media_recuperacao >= media_aprovacao:
            print("A media de recuperação não pode ser maior ou igual que a media de aprovação \n digite novamente: ")
            media_recuperacao = float(input())
        break
    except ValueError: # se o usuario digitar letras ou só espaços
...
print("Digite o sexo do aluno(a): (m) Masculino (f) Feminino")
while True:
    try:
        pronome = str(input())
        if pronome.upper() == "M":
        ...
        print("CAMPO INVALIDO \n Digite novamente: (m) Masculino (f) Feminino")

print(f"Qual o nome d{pronome} alun{pronome} ?")
nome_aluno = input()
while True:
...
print(f"Digite quantas faltas {pronome} alun{pronome} tem: ")
while True:
    try: #inicio de verificação por erro
...
mensagem_inicio = f"{pronome.upper()} alun{pronome} {nome_aluno} esta"
mensagem_aprovado = f"aprovad{pronome}"
mensagem_recuperacao = "de recuperação"
mensagem_reporvado_nota = f"reprovad{pronome} por nota"
mensagem_reprovado_nota_falta = f"reprovad{pronome} por falta e nota"
mensagem_provado_falta = f"reprovad{pronome} por falta"
...
if faltas_aluno < faltas_para_reprovacao: # verifica se o aluno teve a quantidade de aulas minimas 
...
print(f"Media d{pronome} alun{pronome}: {media_aluno}")
